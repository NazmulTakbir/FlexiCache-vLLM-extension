...
from .flexi_cache_triton_kernels import *
...
def chunked_prefill_paged_decode(
    ...
    block_scores=None,
    layer_number=None,
    num_decode_step=None,
    rank_frequency=None,
    max_filtered_blocks=None,
    minmax_block_table=None,
    minmax_key_cache=None,
    top_k_blocks=None,
    old_top_k_blocks=None,
    needs_rerank_gpu=None,
    any_needs_rerank=None,
    last_ranked_num_blks=None,
    is_first_decode_gpu=None,
    any_first_decode=None,
):
    if enable_flexicache:
        block_table        = block_table[:, layer_number, :, :]
        minmax_block_table = minmax_block_table[:, layer_number, :, :]
    if sm_scale is None:
    ...
    block_size = value_cache.shape[2] if enable_flexicache else value_cache.shape[3]
    ...
    if "fp8" in kv_cache_dtype:
        assert not enable_flexicache, "FP8 not supported with flexicache"
    ...
    # TODO (TAKBIR): Need to determine when 2D kernel is faster for FlexiCache
    use_3d_kernel = max_seq_len >= 256 if not enable_flexicache else True

    if not use_3d_kernel:
        # --- 2D KERNEL PATH (SHORT SEQUENCES) ---
        if enable_flexicache:
            raise NotImplementedError("FlexiCache is not supported in 2D kernel path.")
        else:
        ...
        MIN_NUM_SEGMENTS = 4
        MAX_NUM_SEGMENTS = 8 # TODO (TAKBIR): Profile with 8 and 32 for flexicache.
        ...
        if enable_flexicache:
            PAGES_PER_TB = 16
            num_blk_total = max_seq_len // block_size + 1
            grid_z = (num_blk_total + PAGES_PER_TB - 1) // PAGES_PER_TB

            grid = (num_seqs, num_kv_heads, grid_z)
            block_scores_layer = block_scores[layer_number]

            unstable_head_mask       = FCC.UNSTABLE_HEAD_MASKS[layer_number]
            ...
            write_top_k_blocks(
                ...
                block_size, max_filtered_blocks, top_k_blocks,
                FCC.LAYER_TO_UNSTABLE_HEADS_TENSOR[layer_number],
                FCC.LAYER_TO_STABLE_HEADS_TENSOR[layer_number],
                is_first_decode_gpu, any_first_decode,
                layer_has_unstable_heads,
            )

            top_k_blocks_layer = top_k_blocks[layer_number]
            kernel_paged_attention_3d_flexicache[(num_seqs, num_kv_heads, MAX_NUM_SEGMENTS)](
                # Positional Tensors/Pointers (12)
                segm_output, segm_max, segm_expsum, query, key_cache, value_cache,
                block_table,
                top_k_blocks_layer,
                seq_lens,
                last_ranked_num_blks,
                unstable_head_mask,
                # Positional Scalars (4)
                sm_scale, num_seqs,
                # Keyword Constexpr Arguments
                num_query_heads=num_query_heads,
                num_queries_per_kv=num_queries_per_kv,
                num_queries_per_kv_padded=num_queries_per_kv_padded,
                ...
                block_table_stride_2=block_table.stride(2),
                top_k_stride_0=top_k_blocks_layer.stride(0),
                top_k_stride_1=top_k_blocks_layer.stride(1),
                top_k_stride_2=top_k_blocks_layer.stride(2),
                ...
                HEAD_SIZE_PADDED=triton.next_power_of_2(head_size),
                x=key_cache.shape[3],
                kc_stride_0=key_cache.stride(0),
                kc_stride_1=key_cache.stride(1),
                kc_stride_2=key_cache.stride(2),
                kc_stride_3=key_cache.stride(3),
                vc_stride_0=value_cache.stride(0),
                vc_stride_1=value_cache.stride(1),
                vc_stride_2=value_cache.stride(2),
                filter_by_query_len=True,
                query_start_len_ptr=query_start_loc,
            ...
            kernel_reduce_segments_flexicache[(num_seqs, num_query_heads)](
                output_ptr=output,
                seq_lens_ptr=seq_lens,
                last_ranked_num_blks=last_ranked_num_blks,
                unstable_mask_gpu=unstable_head_mask,
            ...
            num_layers = FCC.NUM_LAYERS
            assert layer_number <= num_layers-1
            if layer_number == num_layers-1:
                write_top_k_blocks_post(
                    layer_number, num_seqs, block_scores, max_seq_len,
                    block_size, max_filtered_blocks, top_k_blocks, old_top_k_blocks,
                    FCC.LAYER_TO_STABLE_HEADS_TENSOR[layer_number],
                    needs_rerank_gpu, any_needs_rerank,
                )
        else:
            ...
            kernel_reduce_segments[(num_seqs, num_query_heads)](
                output_ptr=output,
                segm_output_ptr=segm_output,
                segm_max_ptr=segm_max,
                segm_expsum_ptr=segm_expsum,
